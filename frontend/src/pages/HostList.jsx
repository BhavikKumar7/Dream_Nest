...
const HostList = () => {
  ...
  return (
    <>
      ...
      <div className="list">
        {hosts.map((host) => (
          <div key={host._id} className="user-card">
            <h3>{host.firstName} {host.lastName}</h3>
            <p>Email: {host.email}</p>
            <p>Role: {host.role}</p>
            <div className="admin-buttons">
              <button onClick={() => handleViewListings(host._id)}>View Listings</button>
              <button onClick={() => handleDelete(host._id)}>Delete Host</button>
            </div>
          </div>
        ))}
      </div>
